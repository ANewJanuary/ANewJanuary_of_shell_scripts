...
def find_keyboards():
    ...
    for path in evdev.list_devices():
        try:
            ...
            # Must have Enter (present on every real keyboard, unlike KEY_TAB
            # which some internal laptop keyboards don't advertise).
            # Exclude anything with relative axes (EV_REL) — that's a mouse or
            # touchpad, not a keyboard.
            if ecodes.KEY_ENTER in keys and ecodes.EV_REL not in caps:
